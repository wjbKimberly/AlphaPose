...
image_dir = "../../examples/video-tmp"
imgnames = []
vidnames = ["../../examples/video-tmp"]
...
#         print(img1,img2)
        ...
        if not os.path.exists("%s/%s_%s.txt"%(vidname,img1_id,img2_id)):
            cmd = "./deepmatching-static %s %s -nt 10 -downscale 3 -out %s/%s_%s.txt > cache"%(img1,img2,vidname,img1_id,img2_id)
            os.system(cmd)
